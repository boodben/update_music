...
class MusicInfoGUI:
    def __init__(self):
        ...
        log_file = os.path.join(parent_dir, "{}.log".format(time.strftime("%Y-%m-%d",time.localtime())))  # log文件的绝对路径
        logging.basicConfig(filename=log_file, level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s',  encoding='utf-8')
        self.logger = logging.getLogger(__name__)
        self.logger.info(f"当前执行路径：{parent_dir}")
        self.window = tk.Tk()
    ...
    def read_music_files(self):
        ...
        parent_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.getcwd())))
        # parent_dir = os.path.dirname(os.path.dirname(os.getcwd()))
        file_csv = os.path.join(parent_dir, 'music.csv')
        if os.path.exists(file_csv):
            current_time = datetime.now().strftime("%Y-%m-%d %H-%M-%S")
            new_filename = os.path.join(parent_dir, "music" + current_time + ".csv")
            try:
                os.rename(file_csv, new_filename)
        ...
        output_file = os.path.join(parent_dir, 'music.csv')  # 输出文件的绝对路径
    ...
    def update_music_info(self):
        ...
        read_path = os.path.dirname(os.path.dirname(os.path.dirname(os.getcwd())))
        # read_path = os.path.dirname(os.path.dirname(os.getcwd()))
